...
class SugiyamaLayout:
    ...
    def _compute_coordinates(self, node_sizes: dict[str, tuple[float, float]] = None):
        ...
        layer_height = 800  # Extremely large for naked eye visibility
        node_spacing = 700  # Very wide horizontal spacing

        for layer_idx, layer in enumerate(self.layers):
            y = layer_idx * layer_height + 200  # Increased top margin

            total_width = len(layer) * node_spacing
            start_x = 200  # Increased left margin

            for pos, node_id in enumerate(layer):
                x = start_x + pos * node_spacing

                size = node_sizes.get(node_id, (400, 350)) if node_sizes else (400, 350)  # Extremely large nodes
...
class SVGRenderer:
  ...
  <pattern id="grid" width="50" height="50" patternUnits="userSpaceOnUse">
    ...
    def _draw_node(self, node: dict, layout: LayoutNode) -> str:
        ...
        # Add default params for any symbol type that might need them
        if "x1" not in params:
            params["x1"] = layout.x + 5
        if "x2" not in params:
            params["x2"] = layout.x + layout.width - 5
        ...
        # Label below symbol with extremely large text for naked eye visibility
        label_y = layout.y + layout.height + 50
        label_svg = f'<text x="{layout.center.x}" y="{label_y}" text-anchor="middle" fill="#2c3e50" font-size="36" font-weight="bold" font-family="Arial, sans-serif">{tag}</text>'
